...
<p>* Record a TV programme trying flashhigh, flashstd and flashlow modes in order until one succeeds. (use the --info option to see what modes are available for a programme).</p>
<blockquote>
<pre>get_iplayer --type=tv --get 123 --modes=flashhigh,flash std,flashlow
 --flvstreamer="/path/to/rtmpdump"</pre>
</blockquote>
<p>* Using rtmpdump tool to get high quality Flash version of a BBC Radio programme (--flvstreamer is not required if your options tell get_iplayer where rtmpdump is located). Note: Possible modes are (in order of quality): n95_3g, n95_wifi, flashlow, flash std, flashhigh, flashvhigh, flashhd. <a href="http://beebhack.wikia.com/wiki/IPlayer_TV#Comparison_Table">See this page for more details</a>.</p>

<blockquote>
<pre>get_iplayer --type=radio --get 12345 --modes=flashaudio
 --flvstreamer="/path/to/rtmpdump"</pre>
</blockquote>
<p>* Record a radio programme trying flash audio then flashaac modes in order until one succeeds.</p>
<blockquote>
<pre>get_iplayer --type=radio --get 12345
 --modes=flashaudio,flashaac
 --flvstreamer="/path/to/rtmpdump"</pre>
</blockquote>
...
<p>* Stream a programme in mplayer, or xine while recording it to disk (does not yet work in win32, does not work for flash based streams. Works for flash, podcast and realaudio modes):</p>
<blockquote>
...
<p>* All modes other than http will not work through a web proxy. To make only the meta-data web requests go through the proxy add the --partial-proxy option. This will only work if your network allows direct access to the streaming port (e.g. port 1935).</p>
