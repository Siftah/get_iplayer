...
I have implemented some new tagging code in my fork of @get_iplayer@.  This new code enhances the existing tagging functionality as much as possible and provides an optional mechanism to harmonise MP3 and MP4 tagging.

The new tagging functionality is encapsulated primarily in the @Tagger@ class that can be found at the end of the @get_iplayer@ script.  This code was culled from an earlier abandoned experiment to convert my existing Python/Mutagen tagging application into a full-blown tagging library for @get_iplayer@, described here:
...
Aside from the additional tags available, there are two notable differences from the previous scheme:
